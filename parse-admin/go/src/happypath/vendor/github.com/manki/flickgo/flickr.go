...
func getSizesURL(c *Client, args map[string]string) string {
	a := clone(args)
	return makeURL(c, "flickr.photos.getSizes", a, true)
}

// Implements https://www.flickr.com/services/api/flickr.photos.getSizes.html
func (c *Client) GetSizes(args map[string]string) (*SizeResponse, error) {
	r := struct {
		Stat  string       `xml:"stat,attr"`
		Err   flickrError  `xml:"err"`
		Sizes SizeResponse `xml:"sizes"`
	}{}
	if err := flickrGet(c, getSizesURL(c, args), &r); err != nil {
		return nil, err
	}
